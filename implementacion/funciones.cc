...
bool change_patient_menu()
{
  ...
  string fullname_aux, fullname_old, address_aux, phone_aux, birthday_aux, insurance_aux, linea;
  while(getline(fe, linea, ','))
  {
    ...
    if(normalize(fullname)==fullname_aux)
    {
      ...
      address=address_aux;
      phone=phone_aux;
      birthday=birthday_aux;
      insurance=insurance_aux;
    }
  }
  ...
  switch (option)
  {
    case 1:
    {
      ...
      getline(cin, fullname); //para quitar el /n que esta en el buffer
      getline(cin, fullname);
      if(find_patient(normalize(fullname))==true)
      {
        cout<<"El paciente con el nombre "<<fullname<<" ya se encuentra registrado"<<endl;
    ...
    case 2:
    {
      ...
      getline(cin, address);  //para quitar el /n que esta en el buffer
      getline(cin, address);
    }break;
  ...
  Patient p("auxiliar");
  p.change_patient(fullname_old, fullname, address, phone, birthday, insurance);
  return true;
}
