...
string check_date(string date, string start, string end)
{
  ...
  while(getline(in,linea,';'))
  {
    ...
    if(linea==date)
    {
      ...
      if( ((start_min>=start_aux_min) && (start_min<=end_aux_min)) || ((end_min>=start_aux_min) && (end_min<=end_aux_min)) )
      {
...
bool add_date_menu(){
  ...
  if(find_patient(normalize(fullname))==true){
    ...
    while(condition!="false")
    {
      ...
      getline(cin,day);
      if( (day[2]!='/') && (day[5]!='/') ){ //faltan controles
        cout<<"Error en la hora. Ejemplo: 24/03/2020";
        return false;
      }

      cout<<"Inserte la hora de inicio de la cita(HH:MM)"<<endl;
      getline(cin,ini);
      if( (ini[2]!=':') ){ //faltan controles
      ...
      getline(cin,end);
      if( (end[2]!=':') ){ //faltan controles
        cout<<"Error en la hora. Ejemplo: 18:30";
        return false;
      }
      //Comprobar que la hora de inicio es inferior a la de fin
